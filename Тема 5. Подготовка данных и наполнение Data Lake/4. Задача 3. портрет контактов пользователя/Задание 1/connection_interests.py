#/usr/lib/spark/bin/spark-submit --master yarn --deploy-mode cluster --conf spark.executor.memory=2g --conf spark.executor.cores=2 --conf spark.executor.instances=4 --conf spark.dynamicAllocation.enabled=false connection_interests.py 2022-05-25 7 /user/nickperegr/data/events /user/nickperegr/data/analytics/user_interests_d28 /user/master/data/snapshots/tags_verified/actual /user/master/data/snapshots/channel_admins/actual /user/nickperegr/data/analytics/connection_interests_d28_test
...
def main():
    ...
    channel_admins_path = sys.argv[6]
    output_base_path = sys.argv[7]
    ...
    subs_interests = get_subs_interests(posts, subscriptions, verified_tags, channel_admins).cache()
...
def get_contacts(direct_messages):
    users = direct_messages.select(F.explode(F.array(F.col("event.message_from"), F.col("event.message_to"))).alias("user_id"))\
    .distinct()\
    .cache()

    contacts = users\
    .join(direct_messages.select(F.col("event.message_from").alias("from"),
    F.col("event.message_to").alias("to")), 
    (F.col("user_id") == F.col("from")) | (F.col("user_id") == F.col("to")))\
...
def get_subs_interests(posts, subscriptions, verified_tags, channel_admins):
    post_tags = posts\
    .select(F.col("event.message_channel_to").alias("channel_id"),
    F.col("event.tags").alias("tags"))

    verified_sub_tags = post_tags\
    .join(channel_admins, "channel_id")\
    .join(subscriptions,
    (F.col("event.subscription_channel") == F.col("channel_id")))\
    .select(F.col("event.user").alias("user_id"), F.col("tags"), F.col("channel_id"))\
    .dropDuplicates(["user_id", "channel_id"])\
    ...
    .withColumn("rank", F.row_number().over(Window.partitionBy("user_id").orderBy(F.desc("tag_count"))))\
    .where("rank <= 3")\
    .groupBy("user_id")\
    .pivot("rank", [1, 2, 3])\
    .agg(F.first("tag"))\
...
def add_tag_usage_count(df, key):
    ...
    cols = result_columns()
    for c in cols:
        res = res.withColumn(c+'_count', F.count(c).over(Window.partitionBy(key)))
    return res    

def add_tag_rank(df, key):
    res = df
    for c in result_columns():
        res = res.withColumn(c+"_rank", F.row_number().over(Window.partitionBy(key).orderBy(c+"_count")))
    return res

def top_direct_tag(column):
    return F.first(F.when(F.col(column+"_rank") == 1, F.col(column))).alias('direct_'+column)
