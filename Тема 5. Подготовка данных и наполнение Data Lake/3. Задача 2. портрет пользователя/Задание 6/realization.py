#/usr/lib/spark/bin/spark-submit --master yarn --deploy-mode cluster realization.py 2022-05-04 5 /user/nickperegr/data/events /user/nickperegr/5.3.5/analytics
...
def input_event_paths(date, days_count, events_base_path):
        ...
        return [f"{events_base_path}/date={(dt-datetime.timedelta(days=x)).strftime('%Y-%m-%d')}" for x in range(days_count)]

def tag_tops(date, days_count, events_base_path, spark):
	
    message_paths = input_event_paths(f"{date}", int(days_count), f"{events_base_path}")

    result = spark.read\
    .option("basePath", f"{events_base_path}")\
...
def reaction_tag_tops(date, days_count, events_base_path, spark):
	
    reaction_paths = input_event_paths(f"{date}", int(days_count), f"{events_base_path}")
...
def calculate_user_interests(date, days_count, events_base_path, output_base_path, spark):

    post_tops = tag_tops(date, days_count, events_base_path, spark)
    
    reaction_tops = reaction_tag_tops(date, days_count, events_base_path, spark)
    
    result = post_tops.join(reaction_tops, "user_id", "full_outer")

    result.write.mode("overwrite").parquet(f"{output_base_path}/user_interests_d{days_count}/date={date}")
...
def main():
    ...
    calculate_user_interests(date, days_count, events_base_path, output_base_path, spark)    
